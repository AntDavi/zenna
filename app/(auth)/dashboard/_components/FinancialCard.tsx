...
export function FinancialCard({
    ...
    // Cor da porcentagem
    const percentColor = percentChange > 0 ? 'text-green-500' : percentChange < 0 ? 'text-red-500' : 'text-muted-foreground';
    const percentPrefix = percentChange > 0 ? '+' : '';

    return (
        <Card
            className={`bg-gradient-card shadow-soft border-border hover:shadow-medium transition-all duration-300
                ${type === 'income' ? 'border-l-4 border-l-income' : ''}
                ${type === 'expense' ? 'border-l-4 border-l-expense' : ''}
                ${type === 'balance' ? value < 0 ? 'border-l-4 border-l-expense' : 'border-l-4 border-l-income' : ''}
        ...
        >
            ...
            <CardContent>
                ...
                <p className={`text-xs flex items-center gap-1 mt-1 ${percentColor}`}>
                    <span>{percentPrefix}{percentChange}%</span>
